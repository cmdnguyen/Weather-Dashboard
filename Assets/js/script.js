...
function populateSearchHistory() {
  ...
  for (let i = 0; i < myStorage.length; i++) {
    ...
    var storedCity = myStorage[i]
    searchButton.innerHTML = storedCity
...
function fetchWeather(cityToSearchFor) {
  ...
  var weatherURL = "https://api.openweathermap.org/data/2.5/forecast?q=" + cityToSearchFor + "&units=imperial&appid=" + APIKey;
  ...
  fetch(weatherURL)
    ...
    .then(function (data) {
      console.log(data)
      saveSearchHistory(cityToSearchFor)
      ...
      currentCityName.textContent = cityName.value;
...
//if valid
var validateSearchInput
  fetchWeather(validateSearchInput)
